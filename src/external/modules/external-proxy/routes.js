...
const externalUrl = new URL(config.services.external)
const proxyDefaults = {
...
const routes = [
  {
    ...
    handler: {
      proxy: {
        uri: `${externalUrl.protocol}//${externalUrl.hostname}:${externalUrl.port}/{tail}{query}`,
        ...proxyDefaults
      }
    },
    config: {
      auth: false,
      description: 'Proxies requests to Water Abstraction External'
    }
  },
  {
    method: 'GET',
    ...
    handler: {
      proxy: {
        uri: `${externalUrl.protocol}//${externalUrl.hostname}:${externalUrl.port}/assets/all.js`,
        ...proxyDefaults
      }
    },
    config: {
      auth: false,
      description: 'Proxies JS asset requests to Water Abstraction External'
    }
  },
  {
    method: 'GET',
    ...
    handler: {
      proxy: {
        uri: `${externalUrl.protocol}//${externalUrl.hostname}:${externalUrl.port}/assets/stylesheets/application.css`,
        ...proxyDefaults
      }
    },
    config: {
      auth: false,
      description: 'Proxies CSS asset requests to Water Abstraction External'
    }
  },
  {
    method: 'POST',
    // This will match all path segments after /ext. Note in our proxy URI we refer to this only as {tail}. The path
    // param hapi provides will contain all the segments, for example, request.params.tail === '/test/supplementary'.
    // We need to refer to it in the same way we access it on the request object.
    path: '/ext/{tail*}',
    handler: {
