...
const controller = require('../../../../src/external/modules/notifications-reports/controller');
const { events, taskConfig, notifications } = require('../../../../src/external/lib/connectors/water');
...
experiment('getNotification', () => {
  ...
  test('adds the back URL to the view context', async () => {
    const [, viewContext] = h.view.lastCall.args;
    expect(viewContext.back).to.equal('/admin/notifications/report');
  });
});
