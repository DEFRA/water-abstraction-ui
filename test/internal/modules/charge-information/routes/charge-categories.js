...
experiment('internal/modules/charge-information/routes', () => {
  ...
  experiment('getChargeCategoryStep', () => {
    ...
    test('has the expected pre handlers', async () => {
      ...
      expect(routes.getChargeCategoryStep.options.pre[1].method).to.equal(preHandlers.loadDefaultCharges);
      expect(routes.getChargeCategoryStep.options.pre[1].assign).to.equal('defaultCharges');
  ...
  experiment('postChargeCategoryStep', () => {
    ...
    test('has the expected pre handlers', async () => {
      ...
      expect(routes.postChargeCategoryStep.options.pre[1].method).to.equal(preHandlers.loadDefaultCharges);
      expect(routes.postChargeCategoryStep.options.pre[1].assign).to.equal('defaultCharges');
      expect(routes.postChargeCategoryStep.options.pre[2].method).to.equal(preHandlers.loadDraftChargeInformation);
      expect(routes.postChargeCategoryStep.options.pre[2].assign).to.equal('draftChargeInformation');
    });
  });
});
