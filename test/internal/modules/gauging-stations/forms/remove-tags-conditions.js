...
const { form: removeTagsConditionsForm, schema: removeTagsConditionsSchema } = require('internal/modules/gauging-stations/forms/remove-tag/remove-tags-conditions');

experiment('internal/modules/gauging-stations/forms/remove-tags-conditions.js', () => {
  let request, form;
  const data = {
    data: [
      {
  ...
  experiment('.form', () => {
    beforeEach(async () => {
      ...
      request = {
        path: 'http://example.com/monitoring-stations/123/untagging-licence/remove-tags-conditions',
        view: {
    ...
    experiment('load request', () => {
      beforeEach(async () => {
        form = removeTagsConditionsForm(request);
      });

      test('the form has the correct action attribute', async () => {
        expect(form.action).to.equal('http://example.com/monitoring-stations/123/untagging-licence/remove-tags-conditions');
      });
      ...
      test('the form has a radio button', async () => {
        ...
        expect(selectedLicence.options.widget).to.equal('checkbox');
      });
    });
  });
});
